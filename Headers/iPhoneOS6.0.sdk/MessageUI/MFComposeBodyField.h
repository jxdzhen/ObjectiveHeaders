/**
 * This header is generated by class-dump-z 0.2a.
 * class-dump-z is Copyright (C) 2009 by KennyTM~, licensed under GPLv3.
 *
 * Source: /System/Library/Frameworks/MessageUI.framework/MessageUI
 */

#import "MessageUI-Structs.h"
#import <XXUnknownSuperclass.h> // Unknown library

@class DOMHTMLElement, DOMHTMLDocument, MFGenericAttachmentStore;
@protocol MFMailComposeViewDelegate;

@interface MFComposeBodyField : XXUnknownSuperclass {
	DOMHTMLElement* _body;
	DOMHTMLDocument* _document;
	DOMHTMLElement* _blockquote;
	CGSize _originalSize;
	CGSize _layoutSize;
	BOOL _shouldShowStandardButtons;
	unsigned _isDirty : 1;
	unsigned _forwardingNotification : 1;
	unsigned _replaceAttachments : 1;
	unsigned _isLoading : 1;
	unsigned _needsReplaceImages : 1;
	NSRange _rangeToSelect;
	MFGenericAttachmentStore* _attachmentStore;
	id<MFMailComposeViewDelegate> _mailComposeViewDelegate;
@private
	int _preventLayout;
}
@property(assign) BOOL shouldShowStandardButtons;	// @synthesize=_shouldShowStandardButtons
+(void)addAdditionalItemsToCalloutBar;
-(id)initWithFrame:(CGRect)frame;
-(id)_addInlineAttachmentWithData:(id)data text:(id)text type:(id)type contentID:(id)anId;
-(void)_finishedLoadingURLRequest:(id)request success:(BOOL)success;
-(id)_nodeForAttachmentData:(id)attachmentData text:(id)text type:(id)type;
-(void)_nts_AddDOMNode:(id)node quote:(BOOL)quote baseURL:(id)url emptyFirst:(BOOL)first prepended:(BOOL)prepended;
-(void)_webthread_webView:(id)view tileDidDraw:(id)tile;
-(void)addDOMNode:(id)node quote:(BOOL)quote baseURL:(id)url emptyFirst:(BOOL)first prepended:(BOOL)prepended;
-(void)addMailAttributesBeforeDisplay;
-(void)addMarkupString:(id)string quote:(BOOL)quote baseURL:(id)url emptyFirst:(BOOL)first prepended:(BOOL)prepended;
-(void)addSelectedAttachmentsToPasteboard:(id)pasteboard;
-(void)appendMarkupString:(id)string quote:(BOOL)quote;
-(void)appendQuotedMarkupString:(id)string baseURL:(id)url;
-(BOOL)becomeFirstResponder;
-(void)beginPreventingLayout;
-(void)changeQuoteLevel:(int)level;
-(BOOL)containsRichText;
-(float)contentWidth;
-(void)copy:(id)copy;
-(void)cut:(id)cut;
-(void)dealloc;
-(void)deferredBecomeFirstResponder;
-(void)deleteNode:(id)node;
-(void)deleteRange:(id)range;
-(void)deleteTemporarySelectionMarkersFromDocument:(id)document;
-(void)didUndoOrRedo:(id)redo;
-(id)documentFragmentForPasteboardItemAtIndex:(int)index;
-(BOOL)endEditing:(BOOL)editing;
-(void)endPreventingLayout;
-(void)ensureSelection;
-(id)htmlString;
-(void)htmlString:(id*)string otherHtmlStringsAndAttachments:(id*)attachments charsets:(id*)charsets withAttachmentSource:(id)attachmentSource;
-(void)insertNode:(id)node parent:(id)parent nextSibling:(id)sibling;
-(void)insertNode:(id)node parent:(id)parent offset:(int)offset;
-(void)insertPhotoOrVideoWithInfoDictionary:(id)infoDictionary;
-(id)insertTemporarySelectionMarkersForRange:(id)range;
-(void)invalidate;
-(BOOL)isDirty;
-(BOOL)isForwardingNotification;
-(void)layoutWithMinimumSize;
-(void)paste:(id)paste;
-(id)plainTextAlternative;
-(id)plainTextContentWithAttachmentSource:(id)attachmentSource;
-(void)prependMarkupString:(id)string quote:(BOOL)quote;
-(void)prependMarkupString:(id)string quote:(BOOL)quote baseURL:(id)url emptyFirst:(BOOL)first;
-(void)prependString:(id)string;
-(CGRect)rectOfElementWithID:(id)anId;
-(void)removeBlockQuoteFromTree:(id)tree;
-(void)replaceImages;
-(void)replaceImagesIfNecessary;
-(void)replaceNode:(id)node oldNode:(id)node2;
-(void)restoreSelectionFromTemporaryMarkers;
-(void)restoreSelectionFromTemporaryMarkers:(BOOL)temporaryMarkers;
-(void)saveSelectionForUndo;
-(void)scaleImagesToScale:(unsigned)scale withAttachmentStore:(id)attachmentStore;
-(NSRange)selectedRange;
-(void)setAttachmentStore:(id)store;
-(void)setDirty:(BOOL)dirty;
-(void)setFrame:(CGRect)frame;
-(void)setLayoutInterval:(int)interval;
-(void)setLoading:(BOOL)loading;
-(void)setMailComposeViewDelegate:(id)delegate;
-(void)setMarkupString:(id)string;
-(void)setMarkupString:(id)string baseURL:(id)url quote:(BOOL)quote;
-(void)setPinHeight:(float)height;
-(void)setReplaceAttachmentsWithFilename:(BOOL)filename;
-(void)setSelectedRange:(NSRange)range;
-(void)setSelectionStart:(id)start offset:(int)offset end:(id)end offset:(int)offset4 affinity:(int)affinity;
// declared property setter: -(void)setShouldShowStandardButtons:(BOOL)showStandardButtons;
// declared property getter: -(BOOL)shouldShowStandardButtons;
-(void)splitUpBlockQuotesOverlappingEndOfRange:(id)range;
-(void)splitUpBlockQuotesOverlappingStartOfRange:(id)range;
-(id)temporaryEndingSelectionMarker;
-(void)unscaleImages;
-(void)updateQuoteLevelMenu;
-(id)webThreadWebView:(id)view identifierForInitialRequest:(id)initialRequest fromDataSource:(id)dataSource;
-(id)webThreadWebView:(id)view resource:(id)resource willSendRequest:(id)request redirectResponse:(id)response fromDataSource:(id)dataSource;
-(void)webView:(id)view didFirstVisuallyNonEmptyLayoutInFrame:(id)frame;
-(void)webView:(id)view resource:(id)resource didFailLoadingWithError:(id)error fromDataSource:(id)dataSource;
-(void)webView:(id)view resource:(id)resource didFinishLoadingFromDataSource:(id)dataSource;
-(BOOL)webView:(id)view shouldDeleteDOMRange:(id)range;
-(BOOL)webView:(id)view shouldInsertText:(id)text replacingDOMRange:(id)range givenAction:(int)action;
-(void)webViewDidChange:(id)webView;
-(void)webViewDidDraw:(id)webView;
@end

